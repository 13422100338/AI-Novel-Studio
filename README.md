...
> 当前开发版本：`0.4.0`（V3 Phase 4）。本地优先的长篇记忆与动态上下文内核已经落地；正式正文生成仍属于 Phase 5。

Phase 4 新增：

- L0–L4 分层摘要、人物状态时间线，以及彼此隔离的人物/读者知识边界；
- 带权限、审查状态和时间边界的正典、伏笔与叙事线索账本；
- 分层文风规则和不可被模型覆盖的人工样章；
- SQLite FTS5 中文/英文历史检索、相关人物与人工固定权重、过期记录降权；
- 用户自定义输出 Token 上限、动态输入预算、整块摘要回退和可审查 Context Manifest；
- 模型记忆提取候选的嵌套结构校验，以及显式编辑/晋升的记忆工作区边界。

架构与数据规则见 [长篇记忆与动态上下文内核](docs/architecture/0005-memory-and-context-kernel.md)。
