...
> 当前开发版本：`0.7.0`（V3 Phase 7）。本地优先的长篇记忆、动态上下文、冻结 Brief 交接、流式正文草稿、检查点恢复、审校落库、有界修复、严格模式采用边界，以及只读 Agent 工具检索与追踪查看已经落地。
...
Agent 工具循环说明见 [ADR 0007: Read-only Agent tool loop](docs/architecture/0007-agent-tool-loop.md)。
...
- 草稿采用、放弃与重启恢复。
- UI 服务接入和百章压力测试。
### Phase 7：只读 Agent 工具循环

- 剧情商讨可选 Agent 模式，由模型用 JSON 明确请求只读工具。
...
- Agent 只能输出建议或可审查计划，不直接修改正文、记忆库、设置或导出。
...
- [Read-only Agent tool loop](docs/architecture/0007-agent-tool-loop.md)
