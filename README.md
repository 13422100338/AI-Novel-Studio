...
面向长篇与超长篇小说的本地优先 AI 创作工作台。

> 当前版本：V3 开发预览版。欢迎试用、反馈问题并参与改进。
...
- 设置页面可在简体中文与 English 之间切换，并在下次启动时保留选择。
...
`.venv` 不会上传到 GitHub。虚拟环境包含安装者本机的 Python 路径和二进制依赖，
因此每台电脑都需要在下载项目后自行创建。

### Windows 一键安装（推荐）

1. 在 GitHub 页面选择 `Code → Download ZIP` 并完整解压，或使用 Git 克隆仓库。
2. 双击项目根目录中的 `setup.bat`。
3. 安装完成后双击 `launch.bat` 启动程序。
4. 首次启动后，在设置页面添加模型连接和 API Key。
...
V3 已经具备完整的软件骨架和主要创作闭环，目前以开发预览版形式开放。
不同第三方 API 的兼容性、超长项目压力和复杂写作流程仍在持续验证中。

## 主工作区

![AI Novel Studio 主工作区](docs/assets/product-overview/01-main-workspace.png)
...
![独立剧情商讨窗口](docs/assets/product-overview/05-plot-chat.png)
...
![长篇记忆库](docs/assets/product-overview/02-memory-library.png)
...
- ✅ 细节摘录必须逐字来自本章正文；程序会拒绝模型虚构的摘录。
- ✅ 伏笔、悬念和未决问题只进入独立叙事线索账本，不在压缩前文中重复。
- ✅ 人工修改摘要。
- ✅ 人工晋升为已审查。
...
角色相关信息统一进入每个人物一张的聚合状态卡；读者知识事件则按章节边界聚合成一张大白话摘要卡。Brief、剧情商讨和正文生成读取同一份摘要，减少信息泄漏和重复注入。
...
![分层文风系统](docs/assets/product-overview/03-style-system.png)
...
![模型连接与任务路由](docs/assets/product-overview/04-model-settings.png)
...
- ✅ TXT。
- ✅ Markdown。
- 🗺️ DOCX 导入正在规划中；当前版本请先转换为 TXT 或 Markdown。
- ✅ 根据章节标题自动分章。
- ✅ 可配置自动分卷。
...
- 当前发布方式以 Python 虚拟环境和启动脚本为主。
- 不同 Python 版本和 Windows 环境仍需要更多社区测试。
- 正式 EXE 安装包、版本升级和自动更新属于后续发布计划。
...
当前版本将优先提升核心闭环稳定性、长篇记忆质量和模型行为的可检查性，
确保作者能够看见、审查并撤销关键操作。
